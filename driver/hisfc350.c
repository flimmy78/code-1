...
int show_reg_status(struct hisfc_spi *spi)
{
	unsigned int regval;
	struct hisfc_host *host = (struct hisfc_host *)spi->host;
		hisfc_write(host, HISFC350_CMD_INS, SPI_CMD_RDSR3);
		hisfc_write(host, HISFC350_CMD_CONFIG,
			HISFC350_CMD_CONFIG_SEL_CS(spi->chipselect)
...
int change_reg_status(struct hisfc_spi *spi, int value)
{
	...
	hisfc_write(host, HISFC350_CMD_INS, SPI_CMD_WRSR3);
	hisfc_write(host, HISFC350_CMD_DATABUF0, value);
...
void hw_spi_4addr(struct hisfc_spi *spi)
{
    unsigned int value;
    value =0x2;
