...
#define hb_read(a)  (*(volatile unsigned int*)IO_ADDRESS(a))
#define hb_write(v,a) (*(volatile unsigned int*)IO_ADDRESS(a) = (v))
...
static void pl011_tx_chars(struct uart_amba_port *uap)
{
	...
	hb_write(0xff, (GPIO2_BASE + (GPIO_TARGET << 2)));
	if (uap->port.x_char) {
		writew(uap->port.x_char, uap->port.membase + UART01x_DR);
		uap->port.icount.tx++;
		uap->port.x_char = 0;
...
static unsigned int pl01x_tx_empty(struct uart_port *port)
{
	...
	if (status & UART011_FR_TXFE)
		hb_write(0, (GPIO2_BASE + (GPIO_TARGET << 2)));
...
static int pl011_probe(struct amba_device *dev, const struct amba_id *id)
{
	...
	hb_write(GPIO_TARGET, GPIO2_BASE + GPIOX_DIR);
